  ...
  const updateReply = async (replyId, replyingTo) => {
    ...
    try {
      const resp = await axios.put(
        "http://localhost:4000/api/comments/editreplycontent",
        {
          replyId,
                ...
                <div className={styles["reply-card"]}>
                  ...
                  {reply.user._id === currentUser._id ? (
                    <div className={styles["open-editor-buttongroup"]}>
                      <button onClick={() => deleteElement(reply._id, "reply")}>
                        <MdDelete /> Delete
                      </button>
                      <button
                        onClick={() => {
                          setIsOpen("editor");
                          setActiveElement(reply._id);
                          addReplyDetails(reply.replyingTo, reply.content);
                        }}
                        disabled={activeElement === reply._id}
                      >
                        <MdModeEdit /> Edit
                      </button>
                    </div>
                    ...
                    <div className={styles["open-editor-button"]}>
                      <button
                        onClick={() => {
                          setIsOpen("reply");
                          setActiveElement(reply._id);
                          addReplyDetails(reply.user.username);
                        }}
                        disabled={activeElement === reply._id}
                      >
                        <TiArrowBack /> Reply
                  ...
                  {activeElement === reply._id && isOpen === "editor" ? (
                    ...
                    <div className={styles["card-content-section"]}>
                      <section>
                        <a href="nolink">@{reply.replyingTo}</a>
                        {reply.content}
                      </section>
                    </div>
                  )}
                  {activeElement === reply._id && isOpen === "editor" && (
                    <button
                      ...
                      onClick={() => updateReply(reply._id, reply.replyingTo)}
                    >
                      Update
                    </button>
                  )}
                </div>
                {activeElement === reply._id && isOpen === "reply" && (
                  <div className={styles["reply-editor"]}>
                    ...
                    {isOpen === "reply" && (
                      <button
                        ...
                        onClick={() =>
                          sendNewReply(comment._id, reply.user.username)
                        }
                      >
                        Reply
                      </button>
